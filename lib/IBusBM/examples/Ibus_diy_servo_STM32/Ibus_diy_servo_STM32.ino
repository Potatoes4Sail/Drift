...
setup() {                                                                      // Put your setup code here, to run once:
    ...
    SERVO.SetOutputLimits(-511,
                          511);                                                 // Limit to +/- half of analogRead resolution (0-1023)
    SERVO.SetSampleTime(
            IbusInterval);                                                // Sample at IbusInterval (8ms, 125Hz)

    Serial.begin(115200);                                                             // Start Serial usb connection

    IBUS.begin(Serial1,
               IBUSBM_NOTIMER);                                              // Start Ibus connection on serial1 without timer, call IBUS.loop() in Millisecond loop.

    while (IBUS.cnt_rec ==
           0) {                                                       // Wait until first Ibus messages is recieved
...
loop() {                                                                       // Put your main code here, to run repeatedly:
    ...
    if (CurrentMillis !=
        millis()) {                                                 // Create a 1 KHZ Millisecond loop, compare CurrentMillis counter to the Actual Milliseconds
        ...
        if (MillisCount >
            IbusInterval - 1) {                                          // Create a 125 HZ IbusInterval loop

            bool ARMED = map(IBUS.readChannel(ARMED_chan - 1), 1000, 2000, 0, 1);     // Read and convert ARMED_chan
            int STEER = (IBUS.readChannel(
                    STEER_chan - 1));                               // Read ibus steer input from STEER_chan
            int ANGLE = analogRead(
                    ANGLE_pin);                                            // Read Angle from potentiometer

            Setpoint = map(STEER, 1000, 2000, -511,
                           511);                                 // Convert STEER Value to match PID limits
            Input = map(ANGLE, 0, 1023, -511,
                        511);                                       // Convert ANGLE Value to match feedback PID limits
            SERVO.Compute();                                                              // Calculate PID output

            if (IBUS.cnt_rec ==
                IbusCount) {                                              // Check if IBUS.cnt_rec has updated, disable when no New Ibus messages.
                ...
                if (Serial.available()) {                                                  // Only show debug message if usb is connected
                    for (int i = 0;
                         i < 10; i++) {                                           // Show first 10 ibus channels
                        Serial.print(IBUS.readChannel(i));
                        Serial.print(" ");
                    }
                    Serial.print("Cnt=");
                    Serial.print(
                            IBUS.cnt_rec);                                               // Show Ibus message counter
                    Serial.print(" Armed ");
                    Serial.print(ARMED);                                                     // Show ARMED State
                    Serial.print(" , Angle ");
                    Serial.print(
                            ANGLE);                                                     // Show ANGLE from Potentiometer
                    Serial.print(" , PWM ");
                    Serial.println(
                            int(constrain(Output / 2, -MAX_pwm, MAX_pwm)) * ARMED);    // Convert and Show PWM Value
                }

                digitalWrite(ARMED_pin,
                             ARMED);                                             // Write enable pin according to ARMED state

                if (Output > 0) {                                                           // Drive left
                    Output = int(constrain(Output / 2, MIN_pwm, MAX_pwm)) *
                             ARMED;            // Convert output to analogWrite resolution with PWM limits
                    analogWrite(RPWM_pin, 0);                                                 // Disable RPWM_pin
                    analogWrite(LPWM_pin, Output);                                            // Write PWM on LPWM_pin
                } else {                                                                      // Drive right
                    Output = abs(int(constrain(Output / 2, -MAX_pwm, -MIN_pwm))) *
                             ARMED;;    // Convert output to analogWrite resolution with PWM limits
